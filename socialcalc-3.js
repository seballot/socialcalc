...
//         rowoffset = cliprange.cr1.row; 
         ...
         for (row = cr1.row; row < cr1.row+numrows; row++) {
            for (col = cr1.col; col < cr1.col+numcols; col++) {
               ...
               if (cell.readonly) continue;
               if (saveundo) changes.AddUndo("set "+cr+" all", sheet.CellToString(cell));
               var currectClipCol = cliprange.cr1.col + ((col-cr1.col) % (cliprange.cr2.col - cliprange.cr1.col + 1)); 
               var currectClipRow = cliprange.cr1.row + ((row-cr1.row) % (cliprange.cr2.row - cliprange.cr1.row + 1));
               crbase = SocialCalc.crToCoord(currectClipCol, currectClipRow);
               ...
               if (rest == "all" || rest == "formulas") {
                  ...
                  cell.valuetype = basecell.valuetype;
                  if (cell.datatype == "f") { // offset relative coords, even in sheet references
                     cell.formula = SocialCalc.OffsetFormulaCoords(basecell.formula, col - currectClipCol, row - currectClipRow);
                     }
                  else {
                     cell.formula = basecell.formula;
                     }
